...
| 12 | Random Forests & Importance (paired clf + reg) | Solves the single tree's instability through bagging + random feature subsets, on **both** cases. Paired sections throughout: single-tree vs forest, joint `n_estimators × max_features` 3×3 grid (CV mean + 95% Student's *t* CI half-width annotated per cell, red rectangle marks largest-mean-smallest-CI pick — same approach as nb13 §6), OOB vs CV diagnostic, **four-method feature-importance reconciliation heatmap** (linear coefficient / impurity / permutation / drop-column) — the course-wide reference table that nb15 lifts forward. §5 lands on `(n_estimators=50, max_features='sqrt')` for both cases by CI-overlap + parsimony. Closes with a comprehensive comparison plot showing single tree, forest, and the **Week-2 reference** (LogReg / OLS) on a CV-CI dot plot per case. | nb11 proves single trees overfit on both cases; nb12's variance-reduction motivation lands only after experiencing that instability. nb13 needs bagging as a contrast for boosting. |
