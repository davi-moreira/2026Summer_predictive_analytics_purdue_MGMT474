...
- PAUSE-AND-DO 3 (LLM lab): use the Hugging Face `sentiment` and zero-shot `classifier` pipelines to build a "review-intelligence" table (sentiment + theme + confidence per review) over a set of product reviews, then identify and explain the lowest-confidence (mixed-theme) prediction.
