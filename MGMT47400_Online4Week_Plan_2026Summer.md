...
- Sections (full replication of `lecture_slides/10_deep_learning/10_deep_learning.qmd`; all 26 referenced figures copied into `notebooks/figures/` with original names — `10_*.png`, `cifar100.png`, `pytorch_vs_tensorflow.png`, …):
  ...
  - **Neural Networks — build your intuition**: the four 3Blue1Brown *Deep Learning* chapters (what is a NN, gradient descent, backprop intuitively, backprop calculus), consolidated right after the frameworks section
  - **PyTorch hands-on, end-to-end on FashionMNIST** (follows the official Quickstart = data + buildmodel + optimization + saveloadrun): load data + `DataLoader` + visualize grid → build `NeuralNetwork` (+ layer-by-layer + untrained preview) → train/test loop across epochs (CrossEntropyLoss + SGD) → save `state_dict` → load → predict (single image + random-photo grid colored by correctness); core tutorial pages embedded via `IFrame`
