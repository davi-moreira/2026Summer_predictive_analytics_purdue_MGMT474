...
| 12 | Random Forests & Importance (paired clf + reg) | Solves the single tree's instability through bagging + random feature subsets, on **both** cases. **All RF instantiations across §§4–9 are depth-pinned to nb11's per-case picks (`max_depth=3` clf / `max_depth=5` reg)** so per-tree complexity is held constant. Paired sections throughout: single-tree vs forest, joint `n_estimators × max_features` 3×3 grid (CV mean + 95% Student's *t* CI half-width annotated per cell, red rectangle marks largest-mean-smallest-CI pick — same approach as nb13 §6), OOB vs CV diagnostic, **four-method feature-importance reconciliation heatmap** (linear coefficient / impurity / permutation / drop-column) — the course-wide reference table that nb15 lifts forward. §5 lands on `(n_estimators=50, max_features='sqrt', max_depth=3)` for clf and `(n_estimators=50, max_features=0.5, max_depth=5)` for reg by CI-overlap + parsimony — `max_features` adapts per-case because sqrt over-restricts depth-5 trees on 8-feature California Housing. Closes with a comprehensive comparison plot showing single tree, forest, and the **Week-2 reference** (LogReg / OLS) on a CV-CI dot plot per case. | nb11 proves single trees overfit on both cases; nb12's variance-reduction motivation lands only after experiencing that instability. nb13 needs bagging as a contrast for boosting. |
| 13 | Gradient Boosting (paired clf + reg) | Completes the ensemble trilogy on **both** cases — sequential error correction with `learning_rate × n_estimators` joint tuning (3×3 grid showing CV mean + 95% CI half-width annotated per cell, red rectangle marking the largest-mean-smallest-CI pick), and a five-candidate comparison plot (Reference, Tree, RF, default GBM, tuned GBM) per case. Default GBM with sklearn defaults does not automatically beat RF; the §6 `(lr=0.2, n=200)` pick **CI-clears** nb12's depth-pinned forest on regression by roughly 12 R² points — **the regression ship verdict flips from nb12's RF to nb13's tuned GBM** under nb11's depth pinning, because the depth-limited bagging baseline cannot capture California Housing's non-linear structure that sequential boosting extracts. Closes with the **"leaky features dominate the top" callout** connecting nb09's leakage case studies to boosting's amplification effect. | nb12 establishes the parallel ensemble baseline (bagging reduces variance); nb13 contrasts with sequential approach (boosting reduces bias). nb14 needs the full candidate roster. |
...
- Tune `n_estimators` and `max_features` **jointly** via a 3×3 grid with CV mean + 95% CI half-width annotated per cell; apply the CI-overlap rule with parsimony as the tiebreaker on both cases.
...
3. Solution: joint `n_estimators × max_features` 3×3 grid (CV mean + 95% CI per cell) + extension: OOB vs CV diagnostic (9)  
...
- Sections:
  ...
  - Tuning `n_estimators × max_features` — joint 3×3 grid with CV mean + 95% Student's *t* CI half-width annotated per cell (red rectangle marks the largest-mean-smallest-CI cell); same approach as nb13 §6
