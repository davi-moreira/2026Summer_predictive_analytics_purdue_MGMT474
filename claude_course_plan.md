...
| 15 | Interpretation, Calibration & Decision Quality (Project Improved Model) | permutation_importance, PartialDependenceDisplay, calibration_curve, CalibratedClassifierCV(isotonic), brier_score_loss | 14 (champion selected), 12 (importance reconciliation table), 07 (threshold + cost) | 16 (Day-16 freed for time series) | Merges prior nb15 + nb16 into single classification-stakeholder narrative: importance + PDP + segment errors → threshold/cost (nb07 refresh) → reliability + isotonic calibration → decision-policy paragraph + FN-cost sensitivity. Project Improved Model (M3) due. |
