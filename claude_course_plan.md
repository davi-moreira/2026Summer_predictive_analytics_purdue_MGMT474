...
| 12 | Random Forests + Importance (paired clf + reg) | RandomForestClassifier on breast cancer + RandomForestRegressor on California Housing, **joint `(n_estimators × max_features)` 3×3 tuning grid with CV mean + 95% CI half-width annotated per cell (red rectangle marks the largest-mean-smallest-CI pick)**, permutation_importance, OOB scoring on both, drop-column refit; four-method importance heatmap; CV-CI dot plot with Week-2 references as the floor | 11 (tree instability on both cases) | 13 (bagging baseline for boosting contrast), 15 (importance heatmap + PDP for interpretation) | Parallel ensemble solving single-tree variance on both cases; **four-method feature-importance reconciliation heatmap** (linear coef / MDI / permutation / drop-column); §5 joint grid lands on `(n_estimators=50, max_features='sqrt')` for both cases by CI-overlap + parsimony; comprehensive comparison plot benchmarks single tree, forest, and Week-2 reference |
| 13 | Gradient Boosting (paired clf + reg) | GradientBoostingClassifier on breast cancer + GradientBoostingRegressor on California Housing, joint (n_estimators × learning_rate) tuning heatmap with CV mean + 95% CI half-width annotated per cell (red rectangle marks the largest-mean-smallest-CI pick) | 12 (bagging baseline on both cases) | 14 (needs full candidate roster: Reference, Tree, RF, default GBM, tuned GBM per case) | Sequential ensemble on both cases; bias reduction vs variance reduction contrast; tuned GBM ((lr=0.2, n=200) on regression) lifts into a CI-tie with the forest (RF still ships by parsimony); **leaky-features-dominate-boosting callout** referencing nb09 |
