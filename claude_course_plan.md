...
| 15 | Final Project Milestone 03 Walkthrough — Complex Model + Hyperparameter Tuning + Draft Abstract | **Markdown-only walkthrough** (no code, no PAUSE-AND-DO) — companion read for the `milestone_03_complex_model_and_abstract.md` rubric | 14 (champion committed), 09 (hyperparameter tuning), 08 (CI-overlap rule) | 16 (time-series CV splitter pivot) | Same shape as nb05 §6/§7 and nb10's milestone sections. Sections map directly to the M3 rubric: §0 Prediction Goals, §1a Baseline Replication (M2) with 95% CI, §1b Complex Model + Tuning + CI-overlap rule + final-training step + saved champion_pipeline.joblib + CONFIG.json for M4, §1c Required Visualizations, §2 Draft Abstract (~250 words), Course Case Competition (Kaggle Bank Churn) alignment, and Tips and Common Pitfalls. **No Interpretation, Calibration, or Decision-Quality content.** |
