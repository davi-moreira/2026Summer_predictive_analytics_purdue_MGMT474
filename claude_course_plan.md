...
| 12 | Random Forests + Importance (paired clf + reg) | RandomForestClassifier on breast cancer + RandomForestRegressor on California Housing, permutation_importance, OOB scoring on both, drop-column refit; four-method importance heatmap; CV-CI dot plot with Week-2 references as the floor | 11 (tree instability on both spines) | 13 (bagging baseline for boosting contrast), 15 (importance heatmap + PDP for interpretation) | Parallel ensemble solving single-tree variance on both spines; **four-method feature-importance reconciliation heatmap** (linear coef / MDI / permutation / drop-column); comprehensive comparison plot benchmarks single tree, forest, and Week-2 reference |
