...
| 11 | Decision Trees (paired clf + reg) | DecisionTreeClassifier on breast cancer + DecisionTreeRegressor on California Housing, plot_tree, paired depth sweep with the plot_train_val_curve helper, predicted-vs-actual scatter | 10 (evaluation skills consolidated) | 12 (high-variance problem motivates forests; dual-spine pattern continues) | First non-linear model on both spines; concrete bias-variance demonstration via paired depth sweep; one-SE-rule depth selection on each spine; `_clf` / `_reg` namespace convention introduced here and used through nb15 |
| 12 | Random Forests + Importance (paired clf + reg) | RandomForestClassifier on breast cancer + RandomForestRegressor on California Housing, **all RF instantiations depth-pinned to nb11's per-case picks (`max_depth=3` clf / `max_depth=5` reg)**; **joint `(n_estimators × max_features)` 3×3 tuning grid with CV mean + 95% CI half-width annotated per cell (red rectangle marks the largest-mean-smallest-CI pick)**, permutation_importance, OOB scoring on both, drop-column refit; four-method importance heatmap; CV-CI dot plot with Week-2 references as the floor | 11 (tree instability on both cases) | 13 (bagging baseline for boosting contrast), 15 (importance heatmap + PDP for interpretation) | Parallel ensemble solving single-tree variance on both cases; **four-method feature-importance reconciliation heatmap** (linear coef / MDI / permutation / drop-column); §5 joint grid lands on `(n_estimators=50, max_features='sqrt', max_depth=3)` for clf and `(n_estimators=50, max_features=0.5, max_depth=5)` for reg by CI-overlap + parsimony (`max_features` adapts per-case under the pinned depth); comprehensive comparison plot benchmarks single tree, forest, and Week-2 reference |
| 13 | Gradient Boosting (paired clf + reg) | GradientBoostingClassifier on breast cancer + GradientBoostingRegressor on California Housing, joint (n_estimators × learning_rate) tuning heatmap with CV mean + 95% CI half-width annotated per cell (red rectangle marks the largest-mean-smallest-CI pick) | 12 (bagging baseline on both cases) | 14 (needs full candidate roster: Reference, Tree, RF, default GBM, tuned GBM per case) | Sequential ensemble on both cases; bias reduction vs variance reduction contrast; tuned GBM ((lr=0.2, n=200, d=3) on regression) **CI-clears** nb12's depth-pinned forest (n=50, mf=0.5, d=5) by ~12 R² points — **regression verdict flips from RF (nb12) to tuned GBM (nb13)** under nb11's depth pinning, because depth-limited bagging cannot capture California Housing's non-linear structure that sequential boosting extracts; **leaky-features-dominate-boosting callout** referencing nb09 |
| 14 | Model Selection Protocol + Two Ceremonies (paired clf + reg) | comparison harness (cross_validate with multi-metric scoring), Student's *t* 95% CI helper, verdict helper, money plot for INSIDE/ABOVE/BELOW | 13 (full candidate pool on both spines), 08 (CI vocabulary) | 15 (champion committed for interpretation on both spines) | Formal, fair, reproducible comparison on **both** spines; 5 candidates per spine; **two parallel test-set ceremonies** with explicit singleness-rule callout (one ceremony per project, two in this notebook are pedagogical demos); audit_cv_first.py exception list updated for both ceremony cells |
| 15 | Interpretation, Calibration & Decision Quality (paired clf + reg, M3) | permutation_importance, PartialDependenceDisplay, calibration_curve, CalibratedClassifierCV(isotonic), brier_score_loss; residual analysis + RMSE-by-quintile for regression | 14 (both champions selected — LogReg for clf, tuned GBM for reg), 12 (importance heatmap), 07 (threshold + cost) | 16 (Day-16 freed for time series) | Sections 1–6 paired across both spines (interpretation + error analysis); Section 7 (calibration + threshold + cost) classification-only with explicit framing for reg projects. Both committed champions refit; M3 scaffold problem-type-agnostic. |
...
| 19 | Special Topic: Deep Learning | sklearn.neural_network.MLPClassifier (no PyTorch install needed), figures from `notebooks/figures/dl_*.png` | 18 (gradient-boosted tabular champion) | 20 (course-end horizon module) | Awareness module replicating `lecture_slides/10_deep_learning/10_deep_learning.qmd`: historical arc, frameworks, MLP / CNN / RNN / Transformer structural inventions, four-question rubric for "is DL right for this problem?", honest MLP-vs-GBM comparison on tabular data. Designed for business-undergrad audience. |
...
- **Week 3 (Ensembles):** Trees + class_weight → Forests + Importance Table → Boosting + Leak Callout → Selection + Test Set Ceremony → *Interpretation + Improved Model + Kaggle Strategy (Day 15)*
