...
| 13 | Gradient Boosting (paired clf + reg) | GradientBoostingClassifier on breast cancer + GradientBoostingRegressor on California Housing, joint (n_estimators × learning_rate) tuning heatmap, staged_predict early-stopping curves, max_depth-tuned variant | 12 (bagging baseline on both spines) | 14 (needs full candidate roster: Reference, Tree, RF, default GBM, tuned GBM per spine) | Sequential ensemble on both spines; bias reduction vs variance reduction contrast; tuned GBM (max_depth=5) is the CI-clear regression winner; **leaky-features-dominate-boosting callout** referencing nb09 |
