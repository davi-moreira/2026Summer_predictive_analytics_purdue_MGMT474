...
In both cases, the steps below are the most direct on-ramp. Each step is something you can say to Claude Code in your terminal, in your own words. The bullet text is just the intent.

1. **Clone the repository and open it in Claude Code.** This sounds trivial, but it is the move that unlocks everything else: the assistant can now read `CLAUDE.md`, `DECISIONS.md`, the notebook template, the audit scripts, and every brightspace source page in the repo, all at once.

2. **Ask the assistant to summarize the project to you.** A useful opening prompt is something like: *"Read `CLAUDE.md` and `MGMT47400_Online4Week_Plan_2026Summer.md`, then describe back to me how this course is structured: pedagogy, the 21-notebook arc, the source-of-truth files, and the rules that are non-obvious."* What comes back is your map of the territory and a quick sanity check on whether the assistant has the right context.

3. **Translate the constitution to your domain.** Copy `CLAUDE.md` into your own repo and ask the assistant to adapt it: replace QM47400 with your course code, swap "predictive analytics" for your subject, and ask which of the critical rules and anti-patterns plausibly transfer. Keep the structural elements (project mission, "see also" table, critical rules, anti-patterns, session-end checklist) even if every concrete rule changes.

4. **Adopt one pattern at a time.** I would start with the **render-and-commit ritual** and the **per-artifact source `.md` files**. They cost almost nothing to set up and they remove the two most common failure modes. After that, the **instructor-first notebook workflow** is the highest-leverage addition for any course that uses notebooks or lab worksheets.

5. **Adapt the audit scripts to your invariants.** `scripts/audit_cv_first.py` and `scripts/voice_check_guides.py` are short. Ask the assistant to read both, then to write the analogous scripts for the invariants in *your* course. If you teach causal inference, your audit might enforce that no causal claim is made on observational data without a stated identifying assumption. If you teach writing, your audit might flag passive voice in graded examples. The mechanism is the same: write the rule down, then turn it into a check.

6. **Reuse the bulk-CSV pattern for Brightspace.** Once you have a few quizzes in CSV form, the assistant can produce more in the same schema very quickly. The early hour you spend pinning down the column format pays back across the entire semester.

7. **Send me what you build, and what fails.** I keep updating this page as I learn. If you adapt these patterns and discover a sharper version, I would like to hear about it.
